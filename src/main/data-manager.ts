...
export class DataManager {
  ...
  // COLLATE DATA MANAGEMENT
  /**
   * Load collate data from workspace
   */
  public async loadCollateData(): Promise<CollateData> {
    const filePath = this.getDataFilePath('collate.json');
    return await this.loadDataFile<CollateData>(filePath, {
      resources: [],
      tags: {},
  ...
  /**
   * Save collate data to workspace
   */
  public async saveCollateData(data: CollateData): Promise<void> {
    const filePath = this.getDataFilePath('collate.json');
  ...
  /**
   * Add a resource to collate data
   */
  public async addResource(resource: Omit<Resource, 'id' | 'createdAt' | 'modifiedAt'>): Promise<Resource> {
    const data = await this.loadCollateData();
    ...
    const existingResource = data.resources.find(r => r.url === resource.url);
    if (existingResource) {
      throw new Error(`Resource with URL ${resource.url} already exists`);
    }
    ...
    await this.saveCollateData(data);
    return newResource;
  }

  /**
   * Update a resource in collate data
   */
  public async updateResource(id: string, updates: Partial<Resource>): Promise<Resource> {
    ...
    const updatedResource: Resource = {
      ...oldResource,
      ...updates,
      id: oldResource.id, // Prevent ID changes
      createdAt: oldResource.createdAt, // Preserve creation date
      modifiedAt: new Date().toISOString()
    };
    ...
    if (updates.tags) {
      ...
      for (const tag of oldResource.tags) {
        if (data.tags[tag]) {
          data.tags[tag]--;
          if (data.tags[tag] === 0) {
   ...
   */
  public async addTagToResource(resourceId: string, tag: string): Promise<Resource> {
    const data = await this.loadCollateData();
    const resourceIndex = data.resources.findIndex(r => r.id === resourceId);
    ...
    if (resource.tags.includes(tag)) {
      throw new Error(`Tag "${tag}" already exists on this resource`);
    }
  ...
  /**
   * Remove a tag from a specific resource
   */
  public async removeTagFromResource(resourceId: string, tag: string): Promise<Resource> {
    const data = await this.loadCollateData();
    const resourceIndex = data.resources.findIndex(r => r.id === resourceId);
    ...
    if (!resource.tags.includes(tag)) {
      throw new Error(`Tag "${tag}" does not exist on this resource`);
    }
  ...
  public async renameTag(oldTag: string, newTag: string): Promise<void> {
    ...
    // Validate inputs
    if (!oldTag || !newTag) {
      throw new Error('Both old tag and new tag are required');
    }
    
    if (oldTag === newTag) {
      throw new Error('Old tag and new tag cannot be the same');
    }

    // Check if old tag exists
    if (!data.tags[oldTag]) {
      throw new Error(`Tag "${oldTag}" does not exist`);
    }

    // Get the count of the old tag
    const oldTagCount = data.tags[oldTag];

    // If new tag already exists, we'll merge the counts
    const existingNewTagCount = data.tags[newTag] || 0;

    // Update all resources that have the old tag
    let updatedResourcesCount = 0;
    data.resources.forEach(resource => {
      if (resource.tags.includes(oldTag)) {
        // Replace old tag with new tag
        resource.tags = resource.tags.map(tag => tag === oldTag ? newTag : tag);
        
        // If the new tag already exists on this resource, remove duplicates
    ...
    // Update tag counts
    delete data.tags[oldTag]; // Remove old tag count
    data.tags[newTag] = existingNewTagCount + oldTagCount; // Add counts together
  ...
  public async deleteTag(tag: string): Promise<void> {
    ...
    // Remove tag from global tag counts
    delete data.tags[tag];
  ...
  /**
   * Remove a resource from collate data
   */
  public async removeResource(resourceId: string): Promise<void> {
    ...
    // Update tag counts - decrease for each tag
    for (const tag of resource.tags) {
      if (data.tags[tag]) {
        data.tags[tag]--;
        if (data.tags[tag] === 0) {
