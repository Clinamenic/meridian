...
console.log('🔍 Starting Arweave data migration...');
...
async function migrateArweaveData() {
  return new Promise((resolve, reject) => {
    ...
    filesWithArweave.forEach((file, index) => {
      ...
      // Find corresponding resource in database by UUID
      const query = 'SELECT id, arweave_hashes FROM resources WHERE id = ?';
      
      db.get(query, [file.uuid], (err, row) => {
        if (err) {
          console.error(`❌ Database error for ${file.uuid}:`, err.message);
          errorCount++;
          return;
        }

        if (!row) {
          console.log(`⚠️  Resource not found in database: ${file.uuid}`);
          skippedCount++;
          return;
        }
        ...
        if (row.arweave_hashes) {
          try {
            ...
            console.log(`⚠️  Invalid existing Arweave hashes for ${file.uuid}, resetting`);
            existingHashes = [];
          }
        }
        ...
        db.run(updateQuery, [arweaveHashesJson, file.uuid], function(err) {
          if (err) {
            console.error(`❌ Failed to update ${file.uuid}:`, err.message);
            errorCount++;
          } else {
            console.log(`✅ Successfully migrated ${mergedHashes.length} Arweave hashes for ${file.uuid}`);
            migratedCount++;
          }
